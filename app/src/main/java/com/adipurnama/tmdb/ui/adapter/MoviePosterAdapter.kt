...
import com.adipurnama.tmdb.data.network.ImagesPosters
import com.adipurnama.tmdb.databinding.ItemMoviePosterBinding
 ...
 * @2019
 */
internal class MoviePosterAdapter(itemEvents: (ImagesPosters)->Unit) :
    RecyclerView.Adapter<MoviePosterAdapter.ViewHolder>() {

    private var data: List<ImagesPosters> = ArrayList()
    private val listener= itemEvents

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val layoutInflater = LayoutInflater.from(parent.context)
        val itemBinding = ItemMoviePosterBinding.inflate(layoutInflater, parent, false)
    ...
    inner class ViewHolder(private val binding: ItemMoviePosterBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: ImagesPosters) = with(itemView) {
            binding.data=item
    ...
    fun setListData(data: List<ImagesPosters>) {
        this.data = data
